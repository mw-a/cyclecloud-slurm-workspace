...
URI="https://raw.githubusercontent.com/mw-a/cyclecloud-slurm-workspace/$BRANCH/bicep"
...
wget -O initial_pbs_params.json $URI/files-to-load/initial_pbs_params.json
...
while [ $(/opt/cycle_server/./cycle_server execute --format json "
                        ...
                        ON  MachineType === M.Name &&
                            Region === M.Location
                        WHERE (ClusterName == "OpenOnDemand" || ClusterName == \"$MAIN_CLUSTER_NAME\")" | jq -r ".[] | select(.MachineType == null).Name" | wc -l) != 0 ]; do
