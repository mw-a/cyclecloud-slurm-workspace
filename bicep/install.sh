...
cp "${TEMPLATE_PATH}" "${ADMIN_USER_HOME_DIR}/${MAIN_CLUSTER_NAME}/cluster_template.txt"
cp ccwOutputs.json "${ADMIN_USER_HOME_DIR}/${MAIN_CLUSTER_NAME}/deployment.json"
...
/opt/cycle_server/./cycle_server execute "UPDATE Cloud.Node \
                                          ...
                                          WHERE ClusterName==\"$MAIN_CLUSTER_NAME\" && Name==\"login\""
...
while [ $(/opt/cycle_server/./cycle_server execute --format json "
                        SELECT Name, M.Name as MachineType, MMV.Name as MachineTypeMultiValue FROM Cloud.Node
                        OUTER JOIN Azure.MachineType M
                        ON  MachineType === M.Name &&
                            Region === M.Location
                        OUTER JOIN Azure.MachineType MMV
                        ON  MachineType[0] === MMV.Name &&
                            Region === MMV.Location
                        WHERE (ClusterName == "OpenOnDemand" || ClusterName == \"$MAIN_CLUSTER_NAME\")" | jq -r ".[] | select(.MachineType == null and .MachineTypeMultiValue == null).Name" | wc -l) != 0 ]; do
...
"SELECT AdType, ClusterName, Name, M.AcceleratedNetworkingEnabled AS EnableAcceleratedNetworking
 ...
 WHERE ClusterName==\"$MAIN_CLUSTER_NAME\"" > /tmp/accel_network.txt
